...
const buttonStandings = document.querySelector(".tablelib-api");
const buttonLastGames = document.querySelector(".lastlib-api");
const buttonNextGames = document.querySelector(".nextlib-api");
const buttonNowGames = document.querySelector(".nowlib-api");
...
async function tablePositions() {
  try {
    ...
    const tableLocal = sessionStorage.getItem("tablelibertadores");
    if (tableLocal != null) {
      ...
      const response = await fetch(
        "https://v3.football.api-sports.io/standings?league=13&season=2024",
        options
      );
      const data = await response.json();
      const standings = data.response[0].league.standings;

      sessionStorage.setItem("tablelibertadores", JSON.stringify(standings));
...
function renderTable() {
  ...
  const apiData = JSON.parse(sessionStorage.getItem("tablelibertadores"));
...
async function nextGames() {
  try {
    infoApiContainer.setAttribute("aria-busy", "true");
    const round = sessionStorage.getItem("RoundsLibertadores");
    if (round != null) {
      renderNextGames();
    } else {
      const options = {
      ...
      const response = await fetch(
        "https://v3.football.api-sports.io/fixtures?league=13&season=2024&next=18&timezone=America/Argentina/Buenos_Aires",
        options
      );
      const data = await response.json();
      const resp = data.response;
      sessionStorage.setItem("RoundsLibertadores", JSON.stringify(resp));
      renderNextGames();
    }
  } catch (error) {
    console.log(error);
...
function renderNextGames() {
  infoApiContainer.innerHTML = "";
  const data = JSON.parse(sessionStorage.getItem("RoundsLibertadores"));
...
async function lastGames() {
  try {
    infoApiContainer.setAttribute("aria-busy", "true");
    const round = sessionStorage.getItem("lastGamesLibertadores");
    if (round != null) {
      renderLastGames();
    } else {
      const options = {
      ...
      const response = await fetch(
        "https://v3.football.api-sports.io/fixtures?league=13&season=2024&last=15&timezone=America/Argentina/Buenos_Aires",
        options
      );
      const data = await response.json();
      const resp = data.response;
      sessionStorage.setItem("lastGamesLibertadores", JSON.stringify(resp));
      renderLastGames();
    }
  } catch (error) {
    console.log(error);
...
function renderLastGames() {
  infoApiContainer.innerHTML = "";
  const data = JSON.parse(sessionStorage.getItem("lastGamesLibertadores"));
...
async function getGamesToday() {
  try {
    infoApiContainer.setAttribute("aria-busy", "true");
    const gamesToday = sessionStorage.getItem("gamesTodayLibertadores");
    if (gamesToday != null) {
      renderTodayGames();
    } else {
      const date = new Date();
      ...
      const response = await fetch(
        `https://v3.football.api-sports.io/fixtures?league=13&season=2024&date=${formattedDate}&timezone=America/Argentina/Buenos_Aires`,
        options
      );
      const data = await response.json();
      const resp = data.response;
      sessionStorage.setItem("gamesTodayLibertadores", JSON.stringify(resp));
      renderTodayGames();
    }
  } catch (error) {
    console.log(error);
...
function renderTodayGames() {
  const data = JSON.parse(sessionStorage.getItem("gamesTodayLibertadores"));
  data.sort((a, b) => new Date(a.fixture.date) - new Date(b.fixture.date));
...
async function getGameLive() {
  try {
    ...
    if (gameLive != null) {
      ...
      const response = await fetch(
        `https://v3.football.api-sports.io/fixtures?&live=13&timezone=America/Argentina/Buenos_Aires`,
        options
      );
      const data = await response.json();
      const resp = data.response;
